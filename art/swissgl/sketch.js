...
const points = glsl({}, {size:[100,100], story:100,
                        format:'rgba32f', tag:'points'});

glsl({K: numberColors, seed, FP:`
...
function step() {
    for (let i=0; i<1; ++i)
    glsl({F, worldExtent, repulsion: settings.repulsion/10, inertia: settings.inertia / 10, dt: settings.dt, past: points[1], FP:`
        vec3 wrap(vec3 p) {
